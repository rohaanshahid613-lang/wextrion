    ...
    <div style={{ minHeight: '100vh', background: '#0b0c10', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, maxWidth: '1200px', width: '100%', margin: '0 auto', padding: '32px 40px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '26px', fontWeight: 800, color: '#e8eaf2', marginBottom: '6px' }}>✨ Content Optimizer</h1>
          <p style={{ color: '#7c8099', fontSize: '14px' }}>Generate SEO-optimized Amazon listing content that attracts customers and converts.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: result ? '1fr 1fr' : '1fr', gap: '24px' }}>
          ...
          <div>
            <div style={{ background: 'rgba(19,21,28,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', padding: '24px' }}>
              ...
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: '#7c8099', marginBottom: '6px' }}>Current Product Title *</label>
                <input
                  value={currentTitle}
              ...
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: '#7c8099', marginBottom: '6px' }}>
                  Keywords (one per line)
                  <span style={{ color: '#404357', marginLeft: '8px', fontSize: '11px' }}>Paste from your keyword sheet</span>
                </label>
                <textarea
                  value={keywords}
                  onChange={e => setKeywords(e.target.value)}
                  placeholder="vibration plate&#10;whole body vibration machine&#10;vibration platform&#10;exercise vibration plate&#10;fitness vibration board"
                  rows={6}
              ...
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: '#7c8099', marginBottom: '6px' }}>
                  ...
                  <span style={{ color: '#404357', marginLeft: '8px', fontSize: '11px' }}>Target audience, tone, anything extra</span>
                </label>
                <textarea
                  value={notes}
                  onChange={e => setNotes(e.target.value)}
                  placeholder="Target audience: fitness enthusiasts aged 30-60. Tone: professional but friendly. Emphasize the low noise level and compact design..."
                  rows={3}
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '11px 14px', color: '#e8eaf2', fontSize: '14px', outline: 'none', resize: 'vertical', fontFamily: 'inherit' }}
                />
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={generate} disabled={!currentTitle.trim()}
                  style={{ flex: 1, padding: '13px', background: currentTitle.trim() ? 'linear-gradient(135deg, #4ef4b0, #5e9ef4)' : 'rgba(255,255,255,0.05)', color: currentTitle.trim() ? '#0b0c10' : '#7c8099', border: 'none', borderRadius: '10px', fontWeight: 700, fontSize: '15px', cursor: currentTitle.trim() ? 'pointer' : 'not-allowed' }}>
          ...
          {result && (
            <div>
              {/* Tabs */}
              <div style={{ display: 'flex', gap: '4px', marginBottom: '16px', background: 'rgba(19,21,28,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '10px', padding: '4px' }}>
                {tabs.map(tab => (
                  <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                    style={{ flex: 1, padding: '8px', background: activeTab === tab.key ? 'rgba(94,158,244,0.15)' : 'transparent', border: activeTab === tab.key ? '1px solid rgba(94,158,244,0.3)' : '1px solid transparent', borderRadius: '7px', color: activeTab === tab.key ? '#5e9ef4' : '#7c8099', cursor: 'pointer', fontSize: '12px', fontWeight: activeTab === tab.key ? 700 : 400 }}>
                    {tab.label}
                  </button>
                ))}
              </div>
              ...
              {activeTab === 'title' && (
                <div style={{ background: 'rgba(19,21,28,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: 700, color: '#e8eaf2' }}>Optimized Title</h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={{ fontSize: '11px', color: result.title.length > 200 ? '#f45e5e' : '#4ef4b0' }}>
                        {result.title.length}/200 chars
                      </span>
                      <button onClick={() => copyText(result.title, 'title')}
                        style={{ padding: '6px 14px', background: copied === 'title' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: `1px solid ${copied === 'title' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '6px', color: copied === 'title' ? '#4ef4b0' : '#7c8099', cursor: 'pointer', fontSize: '12px' }}>
                        {copied === 'title' ? '✓ Copied!' : 'Copy'}
                      </button>
                    </div>
                  </div>
                  <p style={{ color: '#c5c9d8', fontSize: '14px', lineHeight: 1.7, background: 'rgba(255,255,255,0.03)', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    {result.title}
                  </p>
                </div>
              )}
              ...
              {activeTab === 'bullets' && (
                <div style={{ background: 'rgba(19,21,28,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: 700, color: '#e8eaf2' }}>Bullet Points</h3>
                    <button onClick={() => copyText(result.bullets.join('\n\n'), 'bullets')}
                      style={{ padding: '6px 14px', background: copied === 'bullets' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: `1px solid ${copied === 'bullets' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '6px', color: copied === 'bullets' ? '#4ef4b0' : '#7c8099', cursor: 'pointer', fontSize: '12px' }}>
                      {copied === 'bullets' ? '✓ Copied!' : 'Copy All'}
                    </button>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {result.bullets.map((bullet: string, i: number) => (
                      <div key={i} style={{ background: 'rgba(255,255,255,0.03)', padding: '12px 14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px' }}>
                        <p style={{ color: '#c5c9d8', fontSize: '13px', lineHeight: 1.7, margin: 0, flex: 1 }}>{bullet}</p>
                        <button onClick={() => copyText(bullet, `bullet-${i}`)}
                          style={{ padding: '4px 10px', background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '5px', color: '#7c8099', cursor: 'pointer', fontSize: '11px', flexShrink: 0 }}>
              ...
              {activeTab === 'description' && (
                <div style={{ background: 'rgba(19,21,28,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: 700, color: '#e8eaf2' }}>Product Description</h3>
                    <button onClick={() => copyText(result.description, 'description')}
                      style={{ padding: '6px 14px', background: copied === 'description' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: `1px solid ${copied === 'description' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '6px', color: copied === 'description' ? '#4ef4b0' : '#7c8099', cursor: 'pointer', fontSize: '12px' }}>
                      {copied === 'description' ? '✓ Copied!' : 'Copy'}
                    </button>
                  </div>
                  <pre style={{ color: '#c5c9d8', fontSize: '13px', lineHeight: 1.8, whiteSpace: 'pre-wrap', fontFamily: 'inherit', background: 'rgba(255,255,255,0.03)', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)', maxHeight: '500px', overflowY: 'auto' }}>
                    {result.description}
                  </pre>
                </div>
              )}

              {/* Backend Keywords Tab */}
              {activeTab === 'backend' && (
                <div style={{ background: 'rgba(19,21,28,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: 700, color: '#e8eaf2' }}>Backend Search Terms</h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={{ fontSize: '11px', color: result.backendKeywords.length > 250 ? '#f45e5e' : '#4ef4b0' }}>
                        {result.backendKeywords.length}/250 bytes
                      </span>
                      <button onClick={() => copyText(result.backendKeywords, 'backend')}
                        style={{ padding: '6px 14px', background: copied === 'backend' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: `1px solid ${copied === 'backend' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '6px', color: copied === 'backend' ? '#4ef4b0' : '#7c8099', cursor: 'pointer', fontSize: '12px' }}>
                        {copied === 'backend' ? '✓ Copied!' : 'Copy'}
                      </button>
                    </div>
                  </div>
                  <p style={{ color: '#c5c9d8', fontSize: '13px', lineHeight: 1.7, background: 'rgba(255,255,255,0.03)', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)', wordBreak: 'break-all' }}>
                    {result.backendKeywords}
                  </p>
                  <div style={{ marginTop: '12px', padding: '10px 14px', background: 'rgba(94,158,244,0.06)', border: '1px solid rgba(94,158,244,0.15)', borderRadius: '8px' }}>
                    <p style={{ fontSize: '12px', color: '#7c8099', margin: 0 }}>
                      💡 Paste these into the Search Terms field in your Amazon Seller Central listing editor. Do not repeat words already in your title.
                    </p>
                  </div>
                </div>
              )}
