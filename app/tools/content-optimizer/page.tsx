    ...
    <div style={{ minHeight: '100vh', background: theme.bg, display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 28px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <ThemeSwitcher />
          <Link href="/dashboard" style={{ fontSize: '13px', color: theme.muted, textDecoration: 'none' }}>Back to Dashboard</Link>
        </div>
      </nav>
      ...
      <div style={{ flex: 1, maxWidth: '1200px', width: '100%', margin: '0 auto', padding: '32px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: result ? '1fr 1fr' : '1fr', gap: '24px' }}>
          ...
          <div>
            <div style={{ background: theme.surface, border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', padding: '24px' }}>

              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: theme.muted, marginBottom: '6px' }}>Marketplace</label>
                <select value={marketplace} onChange={e => setMarketplace(e.target.value)}
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '10px 14px', color: theme.text, fontSize: '14px', outline: 'none' }}>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <input value={currentTitle} onChange={e => setCurrentTitle(e.target.value)}
                  ...
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '11px 14px', color: theme.text, fontSize: '14px', outline: 'none' }} />
              </div>

              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: theme.muted, marginBottom: '6px' }}>Keywords (one per line)</label>
                <textarea value={keywords} onChange={e => setKeywords(e.target.value)}
                  placeholder={'vibration plate\nwhole body vibration machine\nvibration platform'}
                  rows={6}
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '11px 14px', color: theme.text, fontSize: '14px', outline: 'none', resize: 'vertical', fontFamily: 'inherit' }} />
              </div>

              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: theme.muted, marginBottom: '6px' }}>Product Features (one per line)</label>
                <textarea value={features} onChange={e => setFeatures(e.target.value)}
                  placeholder={'99 speed levels\nBluetooth remote control\nUltra quiet motor'}
                  rows={6}
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '11px 14px', color: theme.text, fontSize: '14px', outline: 'none', resize: 'vertical', fontFamily: 'inherit' }} />
              </div>

              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: theme.muted, marginBottom: '6px' }}>Additional Notes (Optional)</label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)}
                  placeholder="Target audience, tone, anything extra..."
                  rows={3}
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '11px 14px', color: theme.text, fontSize: '14px', outline: 'none', resize: 'vertical', fontFamily: 'inherit' }} />
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={generate} disabled={!currentTitle.trim()}
                  style={{ flex: 1, padding: '13px', background: currentTitle.trim() ? 'linear-gradient(135deg, ' + theme.accent + ', ' + theme.accent2 + ')' : 'rgba(255,255,255,0.05)', color: currentTitle.trim() ? theme.bg : theme.muted, border: 'none', borderRadius: '10px', fontWeight: 700, fontSize: '15px', cursor: currentTitle.trim() ? 'pointer' : 'not-allowed' }}>
                  Generate Optimized Content
                </button>
                {result && (
                  <button onClick={reset}
                    style={{ padding: '13px 20px', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', color: theme.muted, cursor: 'pointer', fontSize: '14px' }}>
                    Reset
                  </button>
                )}
              </div>
          ...
          {result && (
            <div>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '16px', background: theme.surface, border: '1px solid rgba(255,255,255,0.06)', borderRadius: '10px', padding: '4px' }}>
                {tabs.map(tab => (
                  <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                    style={{ flex: 1, padding: '8px', background: activeTab === tab.key ? 'rgba(255,255,255,0.08)' : 'transparent', border: '1px solid ' + (activeTab === tab.key ? 'rgba(255,255,255,0.15)' : 'transparent'), borderRadius: '7px', color: activeTab === tab.key ? theme.accent : theme.muted, cursor: 'pointer', fontSize: '12px', fontWeight: activeTab === tab.key ? 700 : 400 }}>
                    {tab.label}
                  </button>
                ))}
              </div>
              ...
              {activeTab === 'title' && (
                <div style={{ background: theme.surface, border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <button onClick={() => copyText(result.title, 'title')}
                        style={{ padding: '6px 14px', background: copied === 'title' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: '1px solid ' + (copied === 'title' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'), borderRadius: '6px', color: copied === 'title' ? '#4ef4b0' : theme.muted, cursor: 'pointer', fontSize: '12px' }}>
                        {copied === 'title' ? 'Copied!' : 'Copy'}
                      </button>
                    </div>
                  </div>
                  <p style={{ color: theme.text, fontSize: '14px', lineHeight: 1.7, background: 'rgba(255,255,255,0.03)', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    {result.title}
                  </p>
                </div>
              )}
                ...
                <div style={{ background: theme.surface, border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    ...
                    <button onClick={() => copyText(result.bullets.join('\n\n'), 'bullets')}
                      style={{ padding: '6px 14px', background: copied === 'bullets' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: '1px solid ' + (copied === 'bullets' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'), borderRadius: '6px', color: copied === 'bullets' ? '#4ef4b0' : theme.muted, cursor: 'pointer', fontSize: '12px' }}>
                    ...
                    {result.bullets.map((bullet: string, i: number) => (
                      <div key={i} style={{ background: 'rgba(255,255,255,0.03)', padding: '12px 14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px' }}>
                        ...
                        <button onClick={() => copyText(bullet, 'bullet-' + i)}
                          style={{ padding: '4px 10px', background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '5px', color: theme.muted, cursor: 'pointer', fontSize: '11px', flexShrink: 0 }}>
                ...
                <div style={{ background: theme.surface, border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    ...
                    <button onClick={() => copyText(result.description, 'description')}
                      style={{ padding: '6px 14px', background: copied === 'description' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: '1px solid ' + (copied === 'description' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'), borderRadius: '6px', color: copied === 'description' ? '#4ef4b0' : theme.muted, cursor: 'pointer', fontSize: '12px' }}>
                      {copied === 'description' ? 'Copied!' : 'Copy'}
                    </button>
                  </div>
                  <pre style={{ color: theme.text, fontSize: '13px', lineHeight: 1.8, whiteSpace: 'pre-wrap', fontFamily: 'inherit', background: 'rgba(255,255,255,0.03)', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)', maxHeight: '500px', overflowY: 'auto' }}>
                    {result.description}
                  </pre>
                </div>
              )}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <button onClick={() => copyText(result.backendKeywords, 'backend')}
                        style={{ padding: '6px 14px', background: copied === 'backend' ? 'rgba(78,244,176,0.1)' : 'rgba(255,255,255,0.05)', border: '1px solid ' + (copied === 'backend' ? 'rgba(78,244,176,0.3)' : 'rgba(255,255,255,0.1)'), borderRadius: '6px', color: copied === 'backend' ? '#4ef4b0' : theme.muted, cursor: 'pointer', fontSize: '12px' }}>
                        {copied === 'backend' ? 'Copied!' : 'Copy'}
                      </button>
                    </div>
                  </div>
                  <p style={{ color: theme.text, fontSize: '13px', lineHeight: 1.7, background: 'rgba(255,255,255,0.03)', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)', wordBreak: 'break-all' }}>
                    {result.backendKeywords}
                  </p>
                  <div style={{ marginTop: '12px', padding: '10px 14px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '8px' }}>
